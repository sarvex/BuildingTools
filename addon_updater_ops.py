...
class addon_updater_install_popup(bpy.types.Operator):
	...
	def draw(self, context):
		...
		if updater.invalidupdater == True:
			...
			col.scale_y = 0.7
			col.label(
				text=f"Update {str(updater.update_version)} ready!", icon="LOOP_FORWARDS"
			)
		...
		if updater.manual_only==True:
			...
			if updater.verbose:
				if res==0:
					...
					print(f"Updater returned {res}, error occurred")
...
class addon_updater_check_now(bpy.types.Operator):
	...
	def execute(self,context):
		...
		if not settings:
			if updater.verbose:
				print(f"Could not get {__package__} preferences, update check skipped")
			return {'CANCELLED'}
			...
			try:
				...
				if updater.verbose:
					if res==0:
						...
						print(f"Updater returned {str(res)}, error occurred")
			except Exception as e:
...
class addon_updater_update_target(bpy.types.Operator):
	...
	def target_version(self, context):
		...
		return [(tag, tag, f"Select to install {tag}") for tag in updater.tags]
...
class addon_updater_ignore(bpy.types.Operator):
	...
	@classmethod
	def poll(cls, context):
		return updater.invalidupdater != True and updater.update_ready == True
...
def updater_run_install_popup_handler(scene):
	...
	if "ignore" in updater.json and updater.json["ignore"] == True:
		...
		if ver_tuple < updater.current_version:
			...
			if updater.verbose:
				print(f"{updater.addon} updater: appears user updated, clearing flag")
			updater.json_reset_restore()
			return
	atr = addon_updater_install_popup.bl_idname.split(".")
	getattr(getattr(bpy.ops, atr[0]),atr[1])('INVOKE_DEFAULT')
...
def post_update_callback(module_name, res=None):
	...
	if res is None:
		...
		if updater.verbose:
			print(f"{updater.addon} updater: Running post update callback")
...
def check_for_update_background():
	...
	*Could* be called on register, but would be bad practice.
	...
	# if false, no update ready
	if updater.verbose:
		print(f"{updater.addon} updater: Running background check for update")
...
def update_notice_box_ui(self, context):
	...
	if (
		...
		and "just_updated" in saved_state
		and saved_state["just_updated"] == True
	):
		layout = self.layout
		box = layout.box()
		col = box.column()
...
def update_settings_ui(self, context, element=None):
	...
	if updater.error != None:
		...
		split.operator(
			...
			text=f"Update directly to {str(updater.include_branch_list[0])}",
		)
		split = subcol.split(align=True)
		split.scale_y = 2
		split.operator(addon_updater_check_now.bl_idname,
		...
		split.operator(
			addon_updater_update_now.bl_idname,
			text=f"Update now to {str(updater.update_version)}",
		)
		split = subcol.split(align=True)
		split.scale_y = 2
		split.operator(addon_updater_check_now.bl_idname,
		...
		col.scale_y = 2
		col.operator(
			"wm.url_open", text=f"Download {str(updater.update_version)}"
		).url = updater.website
	...
	if updater.manual_only == False:
		...
		if updater.include_branches == True and len(updater.include_branch_list)>0:
			...
			col.operator(
				...
				text=f"Install latest {branch} / old version",
			)
		else:
			col.operator(addon_updater_update_target.bl_idname,
		...
		backuptext = f"Restore addon backup ({lastdate})"
	...
	elif lastcheck not in ["", None]:
		lastcheck = lastcheck[:lastcheck.index(".")]
		row.label(text=f"Last update check: {lastcheck}")
	else:
...
def update_settings_ui_condensed(self, context, element=None):
	...
	if updater.error != None and updater.error_msg != None:
		...
		lastcheck = lastcheck[:lastcheck.index(".")]
		row.label(text=f"Last check: {lastcheck}")
	else:
...
def skip_tag_function(self, tag):
	...
	if self.version_min_update != None and tupled < self.version_min_update:
		return True # skip if current version below this

	# select the max tag version
	return self.version_max_update != None and tupled >= self.version_max_update
...
def register(bl_info):
	...
	if updater.error:
		print(f"Exiting updater registration, {updater.error}")
		return
	...
	updater.user = "ranjian0"

	# choose your own repository, must match git name
	updater.repo = "building_tools"

	#updater.addon = # define at top of module, MUST be done first

	# Website for manual addon download, optional but recommended to set
	updater.website = "https://ranjian0.github.io/building_tools/"
